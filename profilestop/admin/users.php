...
<?php
	...
	if(isset($_GET['delete']) AND is_numeric($_GET['delete'])) {
		...
		// Silinecek kullanıcıyı veritabanından seç
		$statement = $conn->prepare("SELECT * FROM users WHERE user_id=?");
		$statement->execute(array($_GET['delete']));
		$total = $statement->rowCount();
		
		// Silinecek kullanıcı mevcut değil veya id 1 ise oturumu kapat ve çık
		if($total == 0 OR $_GET['delete'] == 1) {
			...
			// Kullanıcının fotoğrafı varsa ve varsayılan fotoğraf değilse, dosyayı sil
			...
			<div class="card-body">
				<table class="table dataTable table-striped table-hover">
					...
					<tbody>
						...
						<tr>
							<td>
								<!-- Kullanıcının profil fotoğrafını görüntüle -->
							...
							<td>
								<!-- Kullanıcının durumuna bağlı olarak etiket göster -->
								<?php echo ($user['user_status'] == 1) ? "<span class='badge bg-primary me-1 my-1'>Active</span>" : "<span class='badge bg-danger me-1 my-1'>Disabled <span>"; ?>
							</td>
							<td><?php echo date("M d, Y", strtotime($user['user_date_created'])); ?></td>
							<td>
								<!-- Kullanıcıyı düzenleme bağlantısı -->
								<a href="edit-user.php?edit=<?php echo clean($user['user_id']); ?>"><i
										class="align-middle" data-feather="edit-2"></i></a>
								<?php if($user['user_id'] != 1){ ?>
								<!-- Kullanıcıyı silme bağlantısı -->
